...
#    print(dataSet,"dataset")
    ...
    if classList.count(classList[0]) == len(classList):#如果类别完全相同，则停止划分，classList[0]是取其中一个属性
        return classList[0]    #这里采用比较classLebel[0]和classLabel的长度，，若相等，说明这些特征属于同一个
    if len(dataSet[0]) == 1:#说明只剩下了标签项，已经遍历完所有特征时，这时返回出现次数最多的
        return majorityCnt(classList)
    ###########以上为迭代的停止条件###################
    bestFeat = chooseBestFeatureToSplit(dataSet)  #选择一个最好的分类特征,是一个int型数，索引
    bestFeatLabel = labels[bestFeat]  #通过索引获得特征名字，，这里是字符串
